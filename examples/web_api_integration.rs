...
impl AuthHandler {
    ...
    fn register_user_handler(&self, json_body: &str) -> HttpResponse {
        ...
        let username = request_data.get("username").unwrap_or(&String::new()).clone();
        let email = request_data.get("email").unwrap_or(&String::new()).clone();
        let password = request_data.get("password").unwrap_or(&String::new()).clone();
    ...
    fn change_password_handler(&self, json_body: &str) -> HttpResponse {
        let request_data = parse_json(json_body);
        let _old_password = request_data.get("old_password").unwrap_or(&String::new()).clone();
        let new_password = request_data.get("new_password").unwrap_or(&String::new()).clone();
...
fn main() {
    ...
    let register_request = r#"{"username": "testuser", "email": "test@example.com", "password": "MySecureP@ssw0rd!"}"#;
    ...
    let change_pass_request = r#"{"old_password": "oldpass123", "new_password": "NewSecureP@ssw0rd!"}"#;
    ...
    let bad_register_request = r#"{"username": "baduser", "email": "bad@example.com", "password": "password"}"#;
...
mod tests {
    ...
    fn test_web_api_integration() {
        ...
        let request = r#"{"username": "test", "email": "test@test.com", "password": "MySecureP@ssw0rd!"}"#;
        let response = handler.register_user_handler(request);
        assert!(response.contains("success"));
        
        // Test failed registration
        let bad_request = r#"{"username": "test", "email": "test@test.com", "password": "password"}"#;
